...
export interface DataFetcher {
  fetch: <T>(_url: string, _options?: FetchOptions) => Promise<T>;
  fetchSSR: <T>(_url: string, _options?: FetchOptions) => Promise<T>;
  fetchSSG: <T>(_url: string, _options?: FetchOptions) => Promise<T>;
  fetchISR: <T>(
    _url: string,
    _revalidateSeconds: number,
    _options?: FetchOptions,
  ) => Promise<T>;
  useQuery: <T>(_url: string, _options?: FetchOptions) => FetchResult<T>;
  invalidateTag: (_tag: string) => Promise<void>;
...
export function createDataFetcher(): DataFetcher {
  ...
  const determineStrategy = (
    ...
    if (typeof window === "undefined") {
      ...
      return _options?.revalidate ? "isr" : "ssr";
    } else {
      // Client-side: prefer ISR for revalidation, SSG for static
      return _options?.revalidate ? "isr" : "ssg";
    }
  };
