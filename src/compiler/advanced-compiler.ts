...
export interface CodeTransform {
  id: string;
  type: 'layout-shift' | 'useState-to-useReducer' | 'memoization' | 'dependency' | 'bundle';
  description: string;
  before: string;
  after: string;
...
export function createAdvancedCompiler() {
  const baseCompiler = createCompiler();
  ...
  const compile = async (
    ...
    const performanceScore = calculatePerformanceScore(transforms, bundleAnalysis);
    
    // Generate suggestions
    const suggestions = generateSuggestions(transforms, bundleAnalysis, performanceScore);
  ...
  const injectCSSContainment = (element: string): string => {
    if (element.includes('style=')) {
      return element.replace(/style=\{([^}]+)\}/, 'style={{ ...$1, contain: "layout" }}');
    } else {
  ...
  const detectUseStateToUseReducer = (code: string): CodeTransform[] => {
    ...
    const useStateMatches = code.match(/const\s+\[([^,]+),\s*set([^]]+)\]\s*=\s*useState/g);
    
    if (useStateMatches && useStateMatches.length > 2) {
      const transform: CodeTransform = {
        id: 'useState-to-useReducer',
        type: 'useState-to-useReducer',
        description: 'Convert multiple useState to useReducer for better state management',
  ...
  const shouldApplyTransform = (
    ...
    if (transform.impact === 'medium' && context?.performanceBudget > 50) return true;
  ...
  const analyzeBundleImpact = (
    ...
    const packageEstimates: Record<string, {
      size: number;
      tti: number;
      fcp: number;
      lcp: number;
  ...
  const generateSuggestions = (
    ...
    if (performanceScore < 50) {
      suggestions.push('Critical performance issues detected - immediate action required');
    } else if (performanceScore < 75) {
      suggestions.push('Performance improvements needed - review high-impact transforms');
    } else if (performanceScore < 90) {
      suggestions.push('Good performance - consider applying medium-impact optimizations');
    } else {
      suggestions.push('Excellent performance - maintain current optimization level');
    }
    
    // Add specific suggestions based on transforms
    const highImpactTransforms = transforms.filter(t => t.impact === 'high' && !t.applied);
    if (highImpactTransforms.length > 0) {
      suggestions.push(`Apply ${highImpactTransforms.length} high-impact optimizations`);
    }
    
    // Add bundle suggestions
    const notRecommended = bundleAnalysis.filter(a => a.recommendation === 'not-recommended');
    if (notRecommended.length > 0) {
      suggestions.push(`Replace ${notRecommended.length} heavy dependencies with lighter alternatives`);
    }
