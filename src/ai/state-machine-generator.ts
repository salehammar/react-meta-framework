 ...
 */
export function createStateMachineFromPrompt(prompt: string | StateMachinePrompt): GeneratedStateMachine {
  ...
  const complexity = typeof prompt === 'string' ? 'medium' : (prompt.complexity || 'medium');
  ...
  statePatterns.forEach(pattern => {
    ...
    if (matches) {
      matches.forEach(match => {
        const state = match.replace(/(?:should handle|handles?|supports?|manages?|states?|modes?|phases?|like|such as|including)/gi, '').trim();
...
function extractEvents(description: string): string[] {
  ...
  eventPatterns.forEach(pattern => {
    ...
    if (matches) {
      matches.forEach(match => {
        const event = match.replace(/(?:on|when|triggered by|responds to)/gi, '').trim();
...
function generateStateMachineName(description: string): string {
  ...
  const words = description.split(/\s+/).filter(word => 
    word.length > 3 && !['should', 'handle', 'states', 'like', 'such', 'including'].includes(word.toLowerCase())
  );
 ...
 */
function generateTransitions(states: string[], events: string[], context: any, complexity: string) {
...
function generateStateMachineCode(stateMachine: any): string {
  ...
  const eventsType = transitions.map((t: any) => `'${t.event}'`).filter((e: string, i: number, arr: string[]) => arr.indexOf(e) === i).map((e: string) => `'${e}'`).join(' | ');
  
  const transitionsCode = transitions.map((t: any) => {
...
function generateSuggestions(stateMachine: any, context?: string): string[] {
  ...
  if (context?.includes('async')) {
    suggestions.push('Consider adding loading and error states for async operations');
  }
  
  suggestions.push('Add guards to prevent invalid state transitions');
  suggestions.push('Consider adding actions for side effects during transitions');
 ...
 */
export function generateMultipleStateMachines(descriptions: string[]): GeneratedStateMachine[] {
