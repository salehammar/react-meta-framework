...
program
  ...
  .option(
    "-t, --template <template>",
    "Template to use (default, typescript, minimal)",
    "default",
  )
...
program
  ...
  .action(async (templateType, projectName, _options) => {
    try {
...
program
  ...
  .addCommand(
    new Command("design")
      ...
      .option(
        ...
        "Target framework (react, next, remix)",
        "react",
      )
  ...
  .addCommand(
    new Command("api")
      ...
      .action(async (apiSpec, options) => {
        try {
          console.log(
            chalk.blue("🤖 AI Code Generation from API Specification"),
          );
  ...
  .addCommand(
    new Command("suggest")
      ...
      .option(
        "-c, --context <context>",
        "Analysis context (component, hook, api, performance)",
        "component",
      )
      .action(async (codeFile, options) => {
        try {
          ...
          const suggestions = codegen.generateSuggestions(
            code,
            options.context,
          );
  ...
  .addCommand(
    new Command("refactor")
      ...
      .option(
        "-t, --type <type>",
        "Refactoring type (extract-component, extract-hook, optimize-performance, modernize-syntax)",
      ...
      .action(async (codeFile, options) => {
        try {
          ...
          if (options.output) {
            ...
            console.log(
              chalk.green(`✅ Refactored code saved to: ${options.output}`),
            );
          } else {
