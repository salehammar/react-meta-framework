...
export interface ReactiveBackendBinding<T> {
  get: () => T;
  set: (_value: T) => Promise<void>;
  subscribe: (_callback: (_value: T) => void) => () => void;
  sync: () => Promise<void>;
  resolveConflict: (
    _conflictId: string,
    _resolution: "frontend" | "backend" | "merge",
  ) => Promise<void>;
  getConflicts: () => Array<{
    id: string;
    frontendValue: T;
...
export function createReactiveBackendBinding<T>(
  ...
  const syncWithBackend = async (): Promise<void> => {
    try {
      ...
      if (backendValue !== undefined) {
        ...
        // Check for conflicts
        if (
          backendValue !== currentValue &&
          localState.value().lastSync > new Date(0)
        ) {
          const conflict = {
            id: `${key}-${Date.now()}`,
  ...
  const setValue = async (_value: T): Promise<void> => {
    // Update local state immediately
    localState.setValue((prev) => ({ ...prev, value: _value }));

    // Sync to backend
    try {
      await backendConnection.set(key, _value);
  ...
  const resolveConflict = async (
    ...
    const conflict = currentState.conflicts.find((c) => c.id === _conflictId);
    ...
    switch (_resolution) {
      ...
      case "merge":
        resolvedValue = mergeValues(
          conflict.frontendValue,
          conflict.backendValue,
        );
        break;
      default:
        throw new Error(`Unknown resolution: ${_resolution}`);
    }
    ...
    localState.setValue((prev) => ({
      ...
      conflicts: prev.conflicts.map((c) =>
        c.id === _conflictId ? { ...c, resolved: true } : c,
      ),
    }));
 ...
 */
function mergeValues<T>(_frontend: T, _backend: T): T {
  if (typeof _frontend === "object" && typeof _backend === "object") {
    if (Array.isArray(_frontend) && Array.isArray(_backend)) {
      // Merge arrays
      return [...new Set([..._frontend, ..._backend])] as T;
    } else if (
      _frontend &&
      _backend &&
      !Array.isArray(_frontend) &&
      !Array.isArray(_backend)
    ) {
      // Merge objects
      return { ..._backend, ..._frontend } as T;
    }
  }
